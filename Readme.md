...
- [x] 첫 화면에서 이름과 희망 직무가 바로 보이는가?
- [x] 한 줄 소개만 읽어도 어떤 개발자인지 이해할 수 있는가?
- [x] 핵심 역량이 실제 프로젝트 경험과 연결되어 있는가?
- [x] 프로젝트가 문제, 해결 과정, 결과 순서로 설명되어 있는가?
- [x] 중요 정보와 부가 정보의 시각적 우선순위가 구분되어 있는가?
- [x] GitHub, 기술 블로그, 이메일을 쉽게 찾을 수 있는가?
- [x] 채용 담당자가 30초 안에 핵심 정보를 파악할 수 있는가?

### 22.2 콘텐츠 확인

- [x] AI가 임의로 생성한 경력이나 프로젝트가 포함되지 않았는가?
- [x] 프로젝트에서 본인의 역할과 팀의 역할이 구분되어 있는가?
- [x] 기술 스택이 단순 나열되지 않고 사용 이유와 연결되어 있는가?
- [x] 성과 수치와 결과에 확인 가능한 근거가 있는가?
- [x] 플레이스홀더가 실제 정보로 모두 교체되었는가?
- [x] 오탈자와 링크 오류가 없는가?
- [x] 현재 사용하지 않는 임시 문구가 남아 있지 않은가?

### 22.3 디자인 확인

- [x] 나폴나폴의 무드와 컬러가 모든 섹션에서 일관되게 적용되었는가?
- [x] 나비 모티프가 지나치게 유치하거나 장식적으로 보이지 않는가?
- [x] 프로젝트와 핵심 역량이 다른 정보보다 잘 보이는가?
- [x] 제목, 본문, 보조 정보의 크기와 굵기가 구분되는가?
- [x] 문장과 섹션 사이에 충분한 여백이 있는가?
- [x] 사용된 색상이 지나치게 많지 않은가?
- [x] 작은 글자나 낮은 명암으로 가독성이 떨어지는 부분이 없는가?

### 22.4 기능 확인

- [x] 내비게이션을 누르면 정확한 섹션으로 이동하는가?
- [x] 모바일 메뉴가 정상적으로 열리고 닫히는가?
- [x] GitHub, 블로그, 이메일 링크가 정상적으로 작동하는가?
- [x] 이메일 복사 기능과 안내 메시지가 정상적으로 작동하는가?
- [x] 기타 정보의 접기·펼치기 기능이 정상적으로 작동하는가?
- [x] 맨 위로 이동 버튼이 정상적으로 작동하는가?
- [x] JavaScript 오류가 브라우저 콘솔에 나타나지 않는가?

### 22.5 반응형 확인

- [x] 데스크톱 화면에서 콘텐츠가 지나치게 넓게 퍼지지 않는가?
- [x] 태블릿 화면에서 카드와 텍스트가 자연스럽게 배치되는가?
- [x] 모바일 화면에서 가로 스크롤이 발생하지 않는가?
- [x] 모바일에서 버튼과 링크를 손가락으로 쉽게 누를 수 있는가?
- [x] 프로젝트 카드의 텍스트가 잘리거나 겹치지 않는가?
- [x] 모바일 메뉴가 화면을 벗어나지 않는가?

### 22.6 접근성 확인

- [x] 모든 이미지에 내용을 설명하는 alt 속성이 있는가?
- [x] 키보드의 Tab 키로 링크와 버튼에 접근할 수 있는가?
- [x] 현재 포커스된 요소가 시각적으로 표시되는가?
- [x] 버튼에 기능을 설명하는 aria-label이 적용되어 있는가?
- [x] 모바일 메뉴의 열림 상태가 aria-expanded에 반영되는가?
- [x] 색상만으로 정보나 상태를 구분하지 않는가?
- [x] 애니메이션 감소 설정을 고려하였는가?

### 22.7 리소스 및 배포 확인

- [x] 사용한 이미지의 출처와 라이선스를 확인하였는가?
- [x] 사용한 폰트가 무료로 사용 가능한 폰트인가?
- [x] 사용하지 않는 이미지와 코드가 제거되었는가?
- [x] 파일명과 이미지 경로의 대소문자가 일치하는가?
- [x] index.html, style.css, script.js의 연결 경로가 정확한가?
- [x] 배포 환경에서도 이미지와 폰트가 정상적으로 표시되는가?
- [x] 데스크톱과 모바일 브라우저에서 최종 페이지를 확인하였는가?
